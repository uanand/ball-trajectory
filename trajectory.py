...
class pitch:
    ########################################################
    def __init__(self,x,y,z,v_x,v_y,v_z,w,theta,phi,totalTime,fps,mode):
        '''
        ...
        self.status = self.checkValidityOfPitch()
    ########################################################
    ...
    ########################################################
    def checkValidityOfPitch(self):
        eps = 1e-1
        widthOfHomePlate = 0.4318
        strikeZoneHeightLim = [0.5,1.7]
        x,y,z = self.x[-1],self.y[-1],self.z[-1]
        if ((numpy.abs(x)<=widthOfHomePlate) and (numpy.abs(y)<eps) and (z>=strikeZoneHeightLim[0] and z<=strikeZoneHeightLim[1])):
            status = 'strike'
        else:
            status = 'ball'
        return status
...
class pitch_hit:
    ...
    def __init__(self,x,y,z,v_x,v_y,v_z,w,theta,phi,totalTime,fps,mode):
        ...
        self.findTrajectory_pitching(x,y,z,v_x,v_y,v_z,w,theta,phi,mode)
        self.findTrajectory_hitting()
    ...
    ########################################################
    def findTrajectory_pitching(self,x_0,y_0,z_0,v_x_0,v_y_0,v_z_0,w,theta,phi,mode):
        '''
        CALCULATE THE TRAJECTORY OF THE BALL BASED ON INITIAL POSITION,
        VELOCITY, AND SPIN
        '''
        self.counter = 1
        widthOfHomePlate = 0.4318
        while (y_0>widthOfHomePlate and z_0>0):
            x_1,y_1,z_1,v_x_1,v_y_1,v_z_1 = self.positionAfter_dt(x_0,y_0,z_0,v_x_0,v_y_0,v_z_0,w,theta,phi,mode)
            x_0,y_0,z_0 = x_1,y_1,z_1
            v_x_0,v_y_0,v_z_0 = v_x_1,v_y_1,v_z_1
    ...
    ########################################################
    def pitch_hit(self,x_0,y_0,z_0,v_x_0,v_y_0,v_z_0,w,theta,phi,mode):
